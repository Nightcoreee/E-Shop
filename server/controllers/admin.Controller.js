...
export const dashboardStats = catchAsyncError(async (req, res, next) => {
    ...
    const totalRevenueAllTimeResult = await database.query(`SELECT SUM(total_price) FROM orders WHERE paid_at IS NOT NULL`);
    const totalRevenueAllTime = parseFloat(totalRevenueAllTimeResult.rows[0].sum) || 0;
    ...
    const todayRevenueQuery = await database.query(
        `
            SELECT SUM(total_price) FROM orders WHERE paid_at::date = $1 AND paid_at IS NOT NULL
        `,
        [todayDate]
    );
